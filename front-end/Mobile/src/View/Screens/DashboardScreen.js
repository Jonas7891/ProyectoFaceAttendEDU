...
export default function Dashboard() {
  ...
  const formattedDate = new Date().toLocaleDateString(currentLocale, { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' });
        ...
        <View style={styles.container} marginHorizontal={15}>
          ...
          <View style={styles.quickActionsContainer}>
            {menuAcciones.map((item) => (
              <TouchableOpacity key={item.id} style={[styles.quickActionCard, { backgroundColor: colors.card }]} onPress={() => item.screen && console.log(item.screen)} activeOpacity={0.7}>
          ...
          <View style={styles.recentSection}>
            <View style={styles.sectionHeader}>
              ...
              <TouchableOpacity onPress={() => navigation.navigate(isAdmin ? "DisplayingAttendance" : "MiHistorial")}>
            ...
            {(isAdmin ? asistenciasRecientes : misRegistrosRecientes).map((item) => (
              <View key={item.id} style={[styles.recentItem, { backgroundColor: colors.card }]}>
                <View style={styles.recentInfo}>
                  <Text style={[styles.recentName, { color: colors.text }]}>{isAdmin ? item.nombre : item.materia}</Text>
                  <Text style={[styles.recentTime, { color: colors.textSecondary }]}>{isAdmin ? item.hora : `${item.fecha} - ${item.hora}`}</Text>
                </View>
                <View style={[styles.statusBadge, item.estado === 'presente' ? { backgroundColor: colors.statusPresente } : { backgroundColor: colors.statusTarde }]}>
                  <Text style={[styles.statusText, item.estado === 'presente' ? { color: colors.statusTextPresente } : { color: colors.statusTextTarde }]}>
            ...
            {novedades.map((item) => (
              <View key={item.id} style={[styles.novedadCard, { backgroundColor: colors.card }]}>
                ...
                <View style={styles.novedadContent}>
                  ...
                  <Text style={[styles.novedadDescripcion, { color: colors.textSecondary }]}>{item.descripcion}</Text>
