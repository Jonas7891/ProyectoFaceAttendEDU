...
export function AppDataProvider({ children }) {
    ...
    const addStudentFn = useCallback(async (draft) => {
        const updated = await storageAddStudent(students, draft);
        setStudents(updated);
    }, [students]);

    const importStudentsFn = useCallback(async (drafts[])=> {
    ...
    const updateStudentFn = useCallback(async (id, patch) => {
        const updated = students.map(s => s.id === id ? { ...s, ...patch } );
        await saveStudents(updated);
        setStudents(updated);
    }, [students]);

    const removeStudentFn = useCallback(async (id) => {
    ...
    const addUserFn = useCallback(async (draft) => {
        const updated = await storageAddUser(users, draft);
        setUsers(updated);
    }, [users]);

    const updateUserFn = useCallback(async (id, patch) => {
        const updated = await storageUpdateUser(users, id, patch);
        setUsers(updated);
    }, [users]);

    const removeUserFn = useCallback(async (id) => {
...
export function useAppData() {
    ...
    if (!ctx) {
        throw new Error(
            "[FaceAttend] useAppData() debe usarse dentro de . " +
