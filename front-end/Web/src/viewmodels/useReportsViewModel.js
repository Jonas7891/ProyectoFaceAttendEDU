...
import { Period, Student, DailyAttendance, WeeklyAttendance } from "../models/types";
...
function buildPDFHtml(
    ...
    const tableRows = students.map(s => `
            ...
            ${s.status === "active" ? "Activo" : "Inactivo"}</td>
        </tr>
    `).join("");

    const weeklyRows = weeklyData.map(w => `
        ${w.week}</td><td style="text-align:center">${w.rate}%</td></tr>
    `).join("");
...
<meta charset="UTF-8"/>
Reporte de Asistencia — ${periodText}</title>
  ...
  📊 Reporte de Asistencia — FaceAttend EDU</h1>
  <div class="subtitle">Período: ${periodText}</strong> &nbsp;|&nbsp; Generado: ${new Date().toLocaleDateString("es-CO", { dateStyle: "full" })}</div>
  ...
  <div class="section-title">Detalle de Aprendices (${students.length})</div>
    ...
    Código</th>Nombre</th>Programa</th>Ficha/Semestre</th>Asistencia</th>Estado</th></tr></thead>
    ${tableRows}</tbody>
  </table>

  <div class="section-title">Evolución Semanal</div>
  <table style="max-width:320px">
    Semana</th>Tasa</th></tr></thead>
    ${weeklyRows}</tbody>
  </table>
...
export function useReportsViewModel() {
    ...
    const stats = useMemo(() => {
        if (students.length === 0) {
            return [
                ...
                { label: t("Total aprendices"),  value,   color: c.states.success, icon: "users"         },
                { label: t("En riesgo"),         value,   color: c.states.danger,  icon: "alert-circle"  },
                { label: t("Programas"),         value,   color: c.states.warning, icon: "book-open"     },
            ];
        }
        ...
        return [
            {
                ...
                value: `${displayRate}%`, apiKey = == "week" ? 1.2 : period === "month" ? 0.8,
    ...
    const distribution = useMemo(() => {
        if (students.length === 0) return [
            { name: t("A tiempo"),  value, color: c.states.success },
            { name: t("Tardanzas"), value, color: c.states.warning },
            { name: t("Ausentes"),  value, color: c.states.danger  },
        ];
