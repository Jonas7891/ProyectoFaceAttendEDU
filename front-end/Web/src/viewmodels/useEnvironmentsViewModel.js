...
export function useEnvironmentsViewModel() {
    ...
    const registerEnvironment = useCallback(async (form) => {
    ...
    const editEnvironment = useCallback(async (id, form) => {
        const err = validateEnvironmentForm(form);
        if (err) return err;
    ...
    const removeEnvironment = useCallback(async (id)=> {
    ...
    const removeSchedule = useCallback(async (envId, scheduleId)=> {
