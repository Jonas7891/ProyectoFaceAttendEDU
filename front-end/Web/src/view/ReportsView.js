        ...
        <View style={{ gap: 8 }}>
            {data.map((item, i) => (
                <View key={i} style={{ flexDirection: "row", alignItems: "center", gap: 10 }}>
                    ...
                    <Text style={{ fontSize: 10, fontWeight: "700", color: c.text.primary, width: 40, textAlign: "right" }}>
                        {item.rate}%
                    </Text>
                </View>
            ))}
...
function FiltersPanel({ visible, filters, onApply, onReset, onClose, availableCourses }) {
    ...
    return (
        <Modal visible={visible} transparent animationType="fade" onRequestClose={onClose}>
            ...
            <View style={{ position: "absolute", top: 0, right: 0, bottom: 0, width: 360, backgroundColor: c.background.surface, borderLeftWidth: 1, borderLeftColor: c.border.primary, padding: 20 }}>
                ...
                <ScrollView showsVerticalScrollIndicator={false} style={{ flex: 1 }}>
                    ...
                    <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center", paddingVertical: 10, borderTopWidth: 1, borderTopColor: c.border.primary }}>
                        <View>
        ...
        <React.Fragment>
            <ScrollView contentContainerStyle={{ padding: isSmall ? 16 : 24, gap: 16 }} showsVerticalScrollIndicator={false}>
                <PageHeader
                    ...
                    subtitle={t("An�lisis de asistencia por per�odo acad�mico")}
                ...
                <Card padding={14}>
                    <View style={{ flexDirection: "row", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                        <View style={{ flexDirection: "row", alignItems: "center", gap: 6 }}>
                            ...
                            <Text style={{ fontSize: 10, fontWeight: "500", color: c.text.secondary }}>{t("Per�odo:")}</Text>
                        </View>
                ...
                {/* Evoluci�n + Distribuci�n */}
                <View style={{ flexDirection: isSmall ? "column" : "row", gap: 16 }}>
                    <Card style={{ flex: 2 }}>
                        <Text style={{ fontSize: 10, fontWeight: "600", color: c.text.primary, marginBottom: 4 }}>{t("Evoluci�n de asistencia")}</Text>
                    ...
                    <Card style={{ flex: 1 }}>
                        <Text style={{ fontSize: 10, fontWeight: "600", color: c.text.primary, marginBottom: 4 }}>{t("Distribuci�n")}</Text>
                            ...
                            {vm.distribution.map(item => (
                                <View key={item.name} style={{ gap: 6 }}>
                                    <View style={{ flexDirection: "row", justifyContent: "space-between" }}>
                                        <View style={{ flexDirection: "row", alignItems: "center", gap: 8 }}>
                                            <View style={{ width: 10, height: 10, borderRadius: 14, backgroundColor: item.color }} />
                ...
                <View style={{ flexDirection: isSmall ? "column" : "row", gap: 16 }}>
                    <Card style={{ flex: 1 }}>
                        <Text style={{ fontSize: 10, fontWeight: "600", color: c.text.primary, marginBottom: 4 }}>{t("Asistencia por d�a")}</Text>
                ...
                <Card>
                    ...
                    {vm.atRiskStudents.length === 0 ? (
                        <Text style={{ fontSize: 11, color: c.text.secondary, textAlign: "center", paddingVertical: 24 }}>
                            {vm.filtersActive ? t("Ning�n estudiante coincide con los filtros aplicados") : t("No hay estudiantes en riesgo actualmente")}
                        </Text>
                    ) : (
                        <View style={{ gap: 10 }}>
                ...
                <Card>
                    ...
                    <ScrollView horizontal showsHorizontalScrollIndicator={false}>
                        <View>
                            <View style={{ flexDirection: "row", backgroundColor: c.background.app, paddingVertical: 8, paddingHorizontal: 12, borderRadius: 6 }}>
                            ...
                            {vm.filteredStudents.map((s, i) => (
                                <View key={s.id} style={{ flexDirection: "row", paddingVertical: 8, paddingHorizontal: 12, borderBottomWidth: 1, borderBottomColor: c.border.primary }}>
