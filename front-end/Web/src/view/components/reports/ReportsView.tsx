        ...
        <View style={{ gap: 10 }}>
            {data.map(item => (
                <View key={item.day} style={{ flexDirection: "row", alignItems: "center", gap: 10 }}>
                    ...
                    <View style={{ flex: 1, flexDirection: "row", height: 10, borderRadius: 5, overflow: "hidden", backgroundColor: c.border.primary }}>
...
function FiltersPanel({ visible, filters, onApply, onReset, onClose, availableCourses }: {
    ...
    onApply: (f: ReportFilters) => void; onReset: () => void; onClose: () => void;
    ...
    const courseOptions = [{ value: "", label: t("Todos los programas") }, ...availableCourses.map(n => ({ value: n, label: n }))];
                ...
                {options.map(opt => {
                    ...
                    return (
                        <TouchableOpacity key={opt.value} onPress={() => onChange(opt.value)} style={{
                            ...
                            <Text style={{ fontSize: 12, fontWeight: active ? "600" : "400", color: active ? c.text.onBrand : c.text.secondary }}>
                                {opt.label}
                            </Text>
                        </TouchableOpacity>
                    );
    ...
    return (
        <Modal visible={visible} transparent animationType="fade" onRequestClose={onClose}>
            ...
            <View style={{ position: "absolute", top: 0, right: 0, bottom: 0, width: 320, backgroundColor: c.background.surface, borderLeftWidth: 1, borderLeftColor: c.border.primary, padding: 20 }}>
                <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginBottom: 24 }}>
                    <View style={{ flexDirection: "row", alignItems: "center", gap: 8 }}>
                        <Feather name="sliders" size={16} color={c.brand.primary} />
                        <Text style={{ fontSize: 15, fontWeight: "700", color: c.text.primary }}>{t("Filtros")}</Text>
                    </View>
                    <TouchableOpacity onPress={onClose}><Feather name="x" size={18} color={c.text.secondary} /></TouchableOpacity>
                </View>
                <ScrollView showsVerticalScrollIndicator={false} style={{ flex: 1 }}>
                    <Text style={{ fontSize: 12, fontWeight: "600", color: c.text.secondary, textTransform: "uppercase", letterSpacing: 0.8, marginBottom: 8 }}>{t("Programa")}</Text>
                    <ChipRow options={courseOptions} value={draft.courseCode} onChange={v => setDraft(d => ({ ...d, courseCode: v }))} />
                    <Text style={{ fontSize: 12, fontWeight: "600", color: c.text.secondary, textTransform: "uppercase", letterSpacing: 0.8, marginBottom: 8 }}>{t("Estado del estudiante")}</Text>
                    <ChipRow options={statusOptions} value={draft.statusFilter} onChange={v => setDraft(d => ({ ...d, statusFilter: v }))} />
                    <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center", paddingVertical: 12, borderTopWidth: 1, borderTopColor: c.border.primary }}>
                        <View>
                            <Text style={{ fontSize: 13, fontWeight: "500", color: c.text.primary }}>{t("Solo estudiantes en riesgo")}</Text>
                ...
                <View style={{ flexDirection: "row", gap: 10, paddingTop: 16, borderTopWidth: 1, borderTopColor: c.border.primary }}>
        ...
        <>
            <ScrollView contentContainerStyle={{ padding: isSmall ? 16 : 28, gap: 16 }} showsVerticalScrollIndicator={false}>
                <PageHeader
                    ...
                    actions={<>
                        ...
                        <TouchableOpacity onPress={vm.openFilters} style={{
                            ...
                            <Text style={{ fontSize: 12, fontWeight: vm.filtersActive ? "600" : "500", color: vm.filtersActive ? c.brand.primary : c.text.secondary }}>
                        ...
                        {permissions.canExportReports && (
                            <TouchableOpacity onPress={vm.exportPDF} style={{
                                ...
                                <Text style={{ fontSize: 12, fontWeight: "500", color: c.text.secondary }}>{t("Exportar PDF")}</Text>
                            </TouchableOpacity>
                        )}
                        ...
                        {permissions.canExportReports && (
                            <TouchableOpacity onPress={vm.exportExcel} style={{
                                height: 30, paddingHorizontal: 12, borderRadius: 6,
                                alignItems: "center", justifyContent: "center",
                                backgroundColor: c.brand.primary, flexDirection: "row", gap: 6,
                            }}>
                                <Feather name="download" size={13} color={c.text.onBrand} />
                                <Text style={{ fontSize: 12, fontWeight: "600", color: c.text.onBrand }}>{t("Exportar Excel")}</Text>
                            </TouchableOpacity>
                        )}
                    </>}
                />
                    ...
                    <View style={{ flexDirection: "row", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
                        ...
                        {PERIOD_OPTIONS.map(opt => (
                            <TouchableOpacity key={opt.value} onPress={() => vm.setPeriod(opt.value)} style={{
                                ...
                                <Text style={{ fontSize: 12, fontWeight: vm.period === opt.value ? "600" : "400", color: vm.period === opt.value ? c.text.onBrand : c.text.secondary }}>
                                    {t(opt.label)}
                                </Text>
                            </TouchableOpacity>
                        ))}
                        {vm.filtersActive && (
                            <TouchableOpacity onPress={vm.resetFilters} style={{ flexDirection: "row", alignItems: "center", gap: 4, paddingHorizontal: 10, paddingVertical: 4, borderRadius: 99, backgroundColor: c.states.dangerLight }}>
                ...
                <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 12 }}>
                    {vm.stats.map(stat => (
                        <View key={stat.label} style={{ flexBasis: isSmall ? "47%" : "23%", flexGrow: 1 }}>
                            <StatCard label={stat.label} value={stat.value} change={stat.change} changeLabel={stat.changeLabel} color={stat.color} icon={<Feather name={stat.icon as any} size={18} color={stat.color} />} />
                        </View>
                    ))}
                </View>
                ...
                <View style={{ flexDirection: isSmall ? "column" : "row", gap: 16 }}>
                    <Card style={{ flex: 2 }}>
                        <Text style={{ fontSize: 14, fontWeight: "600", color: c.text.primary, marginBottom: 4 }}>{t("Evolución de asistencia")}</Text>
                        <Text style={{ fontSize: 12, color: c.text.secondary, marginBottom: 16 }}>{t("Porcentaje por semana")}</Text>
                        <WeeklySparkline data={vm.attendanceByWeek} />
                    </Card>
                    <Card style={{ flex: 1 }}>
                        <Text style={{ fontSize: 14, fontWeight: "600", color: c.text.primary, marginBottom: 4 }}>{t("Distribución")}</Text>
                        <Text style={{ fontSize: 12, color: c.text.secondary, marginBottom: 16 }}>{t("Estado de asistencia")}</Text>
                        <View style={{ gap: 14 }}>
                ...
                <View style={{ flexDirection: isSmall ? "column" : "row", gap: 16 }}>
                    <Card style={{ flex: 1 }}>
                        <Text style={{ fontSize: 14, fontWeight: "600", color: c.text.primary, marginBottom: 4 }}>{t("Asistencia por día")}</Text>
                        <Text style={{ fontSize: 12, color: c.text.secondary, marginBottom: 16 }}>{t("Esta semana")}</Text>
                        <DailyBars data={vm.attendanceByDay} />
                    </Card>
                    <Card style={{ flex: 1 }}>
                        <Text style={{ fontSize: 14, fontWeight: "600", color: c.text.primary, marginBottom: 4 }}>{t("Ranking por curso")}</Text>
                        <Text style={{ fontSize: 12, color: c.text.secondary, marginBottom: 16 }}>{t("Asistencia promedio")}</Text>
                        <View style={{ gap: 14 }}>
                            {vm.courseRanking.map(item => (
                                <View key={item.code} style={{ flexDirection: "row", alignItems: "center", gap: 12 }}>
                                    <View style={{ width: 22, height: 22, borderRadius: 11, backgroundColor: item.rank === 1 ? c.states.warning : c.interactive.disabled, alignItems: "center", justifyContent: "center" }}>
                                        <Text style={{ fontSize: 11, fontWeight: "700", color: item.rank === 1 ? c.text.onBrand : c.text.secondary }}>{item.rank}</Text>
                                    </View>
                                    <View style={{ flex: 1 }}>
                                        <View style={{ flexDirection: "row", justifyContent: "space-between", marginBottom: 4 }}>
                                            <Text style={{ fontSize: 12, fontWeight: "500", color: c.text.primary, flex: 1 }} numberOfLines={1}>{item.courseName}</Text>
                                            <Text style={{ fontSize: 12, fontWeight: "700", color: item.barColor, marginLeft: 8 }}>{item.rate}%</Text>
                                        </View>
                ...
                <Card>
                    <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
                        <View>
                            <Text style={{ fontSize: 14, fontWeight: "600", color: c.text.primary }}>{t("Estudiantes en riesgo")}</Text>
                            <Text style={{ fontSize: 12, color: c.text.secondary }}>{t("Asistencia por debajo del 75%")}</Text>
                        </View>
                    ...
                    {vm.atRiskStudents.length === 0 ? (
                        ...
                        <View style={{ gap: 10 }}>
                            {vm.atRiskStudents.map(student => (
                                <View key={student.id} style={{ flexDirection: "row", alignItems: "center", gap: 12, padding: 12, backgroundColor: c.states.dangerLight, borderRadius: 8 }}>
                                    <Avatar name={student.name} size={36} color={c.states.danger} />
                                    <View style={{ flex: 1 }}>
                                        <Text style={{ fontSize: 13, fontWeight: "600", color: c.text.primary }}>{student.name}</Text>
                ...
                <Card>
                    <View style={{ marginBottom: 12 }}>
                        <Text style={{ fontSize: 14, fontWeight: "600", color: c.text.primary }}>{t("Todos los aprendices")}</Text>
                    ...
                    <ScrollView horizontal showsHorizontalScrollIndicator={false}>
                        <View>
                            <View style={{ flexDirection: "row", backgroundColor: c.background.app, paddingVertical: 8, paddingHorizontal: 10, borderRadius: 6 }}>
                                {[t("Nombre"), t("Programa"), t("Semestre"), t("Asistencia"), t("Estado")].map((h, w) => (
                                    <Text key={h} style={{ width: [160,160,100,100,90][w], fontSize: 11, fontWeight: "700", color: c.text.secondary, textTransform: "uppercase", letterSpacing: 0.4 }}>{h}</Text>
                                ))}
                            </View>
                            {vm.filteredStudents.map((s, i) => (
                                <View key={s.id} style={{ flexDirection: "row", paddingVertical: 8, paddingHorizontal: 10, borderBottomWidth: 1, borderBottomColor: c.border.primary }}>
                                    <Text style={{ width: 160, fontSize: 12, color: c.text.primary }} numberOfLines={1}>{s.name}</Text>
                                    <Text style={{ width: 160, fontSize: 12, color: c.text.secondary }} numberOfLines={1}>{s.course}</Text>
                                    <Text style={{ width: 100, fontSize: 12, color: c.text.secondary }} numberOfLines={1}>{s.grade}</Text>
                                    <Text style={{ width: 100, fontSize: 12, fontWeight: "700", color: s.attendance < 75 ? c.states.danger : c.states.success }}>{s.attendance}%</Text>
                                    <Text style={{ width: 90, fontSize: 12, color: c.text.secondary }}>{s.status === "active" ? t("Activo") : t("Inactivo")}</Text>
                                </View>
                            ))}
                        </View>
                    </ScrollView>
