/**
 * Componentes específicos del Dashboard
 * 
 * Estos componentes son específicos para el dashboard y no son genéricos.
 * Usan componentes genéricos (BarChart, ProgressBar) para su implementación.
 */

export { DailyBarChart } from './DailyBarChart';
export { WeeklyTrend } from './WeeklyTrend';
export { WeeklyAttendanceView, useWeeklyAttendanceController } from './WeeklyAttendanceView';
export { InstructorAttendanceList } from './InstructorAttendanceList';
export { TopPerformingGroups } from './TopPerformingGroups';
export { AtRiskStudentsList } from './AtRiskStudentsList';
export { PerfectAttendanceList } from './PerfectAttendanceList';
export { MetricsSummary } from './MetricsSummary';
