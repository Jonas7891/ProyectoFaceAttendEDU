...
export function AttendanceBadge({ attendance, showIcon = false }: AttendanceBadgeProps) {
...
export function AttendanceStatusIcon({ status, size = 16 }: AttendanceStatusIconProps) {
    const { theme } = useTheme();
    const c = theme.colors;
...
export function useAttendanceColor(attendance: number): string {
    const { theme } = useTheme();
