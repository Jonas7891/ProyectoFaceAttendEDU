...
class TranslationService {
    ...
    async translate(text, language) {
        ...
        // Nivel 1: cach� en memoria
        const cached = translationCache.get(language, text);
        if (cached !== null) return cached;

        // Nivel 2: storage (solo si a�n no se hidrat� este idioma)
    ...
    //      traducci�n fue exitosa.
    //    - Si el proveedor falla, se lanza el error hacia arriba.
    //    - El llamador (translate()) devuelve el texto original como fallback
    //      visible en la UI, PERO sin guardarlo en cach�.
    //    - En el siguiente render, t() vuelve a intentar la traducci�n.
    //
