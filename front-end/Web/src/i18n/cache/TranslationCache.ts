...
export class TranslationCache {
    // Map<idioma, Map<textoFuente, traducción>>
    private readonly store = new Map<LanguageCode, Map<string, string>>();

    // ── Lectura ──────────────────────────────────────────────

    /** Devuelve la traducción si existe en memoria, null si no. */
    get(language: LanguageCode, source: string): string | null {
        return this.store.get(language)?.get(source) ?? null;
    }

    /** True si la traducción ya está en memoria. */
    has(language: LanguageCode, source: string): boolean {
    ...
    set(language: LanguageCode, source: string, translation: string): void {
     ...
     */
    hydrate(language: LanguageCode, entries: Record<string, string>): void {
        if (!this.store.has(language)) {
            this.store.set(language, new Map());
        }
     ...
     */
    export(language: LanguageCode): Record<string, string> {
        const map = this.store.get(language);
        if (!map) return {};
    ...
    exportAll(): Record<LanguageCode, Record<string, string>> {
    ...
    size(language: LanguageCode): number {
