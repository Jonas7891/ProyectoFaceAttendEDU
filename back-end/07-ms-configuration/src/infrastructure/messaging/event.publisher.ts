...
function getProducer(): ReturnType<Kafka['producer']> | null {
  ...
  if (!producer) {
    const kafka = new Kafka({ clientId: 'configuration-service', brokers });
  ...
  app.addHook('onResponse', async (req, reply) => {
    try {
      if (req.method === 'POST' && reply.statusCode >= 200 && reply.statusCode < 300) {
        await publishEvent('configuration-events', { method: req.method, url: req.url, status: reply.statusCode });
      }
    } catch {
      // ignore
    }
