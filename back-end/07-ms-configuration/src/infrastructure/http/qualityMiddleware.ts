 ...
 * Mismo patrón que 03-ms-academic para consistencia ISO.
 */
...
const metrics: QualityMetrics = {
  totalOperations: 0, successCount: 0, errorCount: 0,
  clientErrors: 0, serverErrors: 0,
  createCount: 0, readCount: 0, updateCount: 0, deleteCount: 0,
  totalDurationMs: 0,
};
...
function classifyISOClause(status: number): string {
  ...
  if (status === 404) return '8.5';
  if (status === 409) return '8.5';
...
export function registerQualityMiddleware(app: FastifyInstance): void {
  app.addHook('onRequest', async (request: FastifyRequest, _reply: FastifyReply) => {
    (request as any).quality = {
      correlationId: crypto.randomUUID(),
      startTime: Date.now(),
      operation: classifyOperation(request.method, request.url),
    };
  });
  ...
  app.addHook('onSend', async (request: FastifyRequest, reply: FastifyReply, payload: string) => {
    ...
    if (isError) {
      metrics.errorCount++;
      if (status >= 500) metrics.serverErrors++; else metrics.clientErrors++;
    } else {
...
export function registerQualityHealthEndpoint(app: FastifyInstance): void {
  app.get('/health/quality', async () => buildReport('07-ms-configuration'));
  app.get('/api/v1/quality/report', async () => buildReport('07-ms-configuration'));
}

function buildReport(serviceName: string) {
  const errorRate = metrics.totalOperations === 0 ? 0.0 : (metrics.errorCount / metrics.totalOperations) * 100;
  const avgResponseTime = metrics.totalOperations === 0 ? 0.0 : metrics.totalDurationMs / metrics.totalOperations;
  const availability = metrics.totalOperations === 0 ? 100.0 : (metrics.successCount / metrics.totalOperations) * 100;
  ...
  return {
    ...
    crud_operations: {
      creates: metrics.createCount, reads: metrics.readCount,
      updates: metrics.updateCount, deletes: metrics.deleteCount,
    },
    ...
    iso_clauses_status: {
      clause_4_4_context: 'ACTIVE', clause_8_2_requirements: 'ACTIVE',
      clause_8_5_production: 'ACTIVE', clause_9_1_measurement: 'ACTIVE',
      clause_10_2_corrective: 'ACTIVE', clause_7_5_documentation: 'ACTIVE',
    },
